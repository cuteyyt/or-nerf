...
masks and [**LaMa**](https://github.com/advimman/lama) to inpaint the unwanted region. Then, scene with objects deleted
...
1. [**IBRNet data:**](https://github.com/googleinterns/IBRNet) We use 8 scenes from this dataset: qq3, qq6, qq10, qq11
   and qq13 from *ibrnet_collected\ibrnet_collected_1*, and qq16, qq17 and qq21 from
   *ibrnet_collected/ibrnet_collected_2*.
2. [**SPIn-NeRF data:**](https://github.com/SamsungLabs/SPIn-NeRF) We use all scenes from *spinnerf-dataset* for
   multiview segmentation and 8 scenes (excluding 1 and 9) for scene object removal. Note their statue scene is actually
   qq11 in IBRNet data.
3. [**NeRF LLFF data:**](https://github.com/bmild/nerf) We use 3 scenes from this dataset: room, horns, fortress from
   *nerf_llff_data*.
4. [**LLFF Real data:**](https://github.com/Fyusion/LLFF) We use only one scene from this dataset: data5_piano from
   *real_iconic*.
