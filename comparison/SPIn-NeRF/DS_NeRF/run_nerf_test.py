...
def train():
    ...
    if args.render_all:
        ...
        with torch.no_grad():
            ...
            for _ in range(len(poses[40:])):
                ...
                cv2.imwrite(f'{out_dirs}/img{_:0>3}.png', img)
                cv2.imwrite(f'{out_disps_dirs}/img{_:0>3}.png', disp)
    ...
    for i in trange(start, N_iters):
        ...
        if i % args.i_feat == 0 and i > 0:  # calculate inpainted depths
            ...
            os.makedirs(f'{basedir}/{expname}/test_renders', exist_ok=True)
            plt.savefig('{}/{}/test_renders/{}_lpips_{}.png'.format(basedir, expname,
            ...
            if args.prepare:
                ...
                for _ in range(len(poses)):
                    cv2.imwrite(
                        f'lama/LaMa_test_images/img{_ + 40:0>3}.png', disps[_].detach().cpu().numpy() * 255)
                    cv2.imwrite(f'lama/LaMa_test_images/label/img{_ + 40:0>3}.png',
