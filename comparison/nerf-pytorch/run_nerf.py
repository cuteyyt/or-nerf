...
from model.nerf import get_embedder, NeRF
from model.rays import get_rays, get_rays_np, ndc_rays, sample_pdf
from utils.utils import img2mse, mse2psnr, to8b
...
def render_path(render_poses, hwf, K, chunk, render_kwargs, savedir=None, render_factor=0):
    ...
    for i, c2w in enumerate(tqdm(render_poses)):
        ...
        if savedir is not None:
            ...
            filename_disp = os.path.join(f'{savedir}_disp', '{:03d}.npy'.format(i))
            np.save(filename_disp, disps)
...
def render_rays(ray_batch,
    ...
    if perturb > 0.:
        ...
        middles = .5 * (z_vals[..., 1:] + z_vals[..., :-1])
        upper = torch.cat([middles, z_vals[..., -1:]], -1)
        lower = torch.cat([z_vals[..., :1], middles], -1)
...
def render(H, W, K, chunk=1024 * 32, rays=None, c2w=None, ndc=True,
           near=0., far=1.,
           use_viewdirs=False, c2w_static_cam=None,
           **kwargs):
    """Render rays
    Args:
      H: int. Height of image in pixels.
      W: int. Width of image in pixels.
      K: float. intrinsic_matrix.
...
def config_parser():
    ...
    parser.add_argument("--datadir", type=str, default='./data/nerf_llff_data_sparse/room',
    ...
    parser.add_argument("--white_bkgd", action='store_true',
                        help='set to render synthetic data on a white bkgd (always use for d-voxels)')
...
def train():
    ...
    images, poses, bds, render_poses, i_test = \
        load_llff_data(args.datadir, args.factor, recenter=True, bd_factor=.75, spherify=args.spherify)
    ...
    if args.render_only:
        ...
        with torch.no_grad():
            ...
            testsavedir = os.path.join(basedir, expname, 'render_only_{}_{:06d}'.format(
    ...
    start = start + 1
    global_step = start
    with tqdm(total=args.N_iters) as t_bar:
        for i in range(start, N_iters):
            ...
            rgb, disp, acc, extras = render(H, W, K, chunk=args.chunk, rays=batch_rays, ret_raw=True,
                                            **render_kwargs_train)
            ...
            if i % args.i_print == 0:
                t_bar.write(f"[TRAIN] Iter: {i} Loss: {loss.item()}  PSNR: {psnr.item()}")
                ...
                logger.add_scalar('train/loss_fine', img_loss_copy, global_step)
            ...
            if i % args.i_img == 0:
                ...
                logger.add_image('val/disp_fine', disp[:, :, None], global_step, dataformats='HWC')
                # logger.add_image('acc', acc[None, ..., None], global_step)
                ...
                if args.N_importance > 0:
                    ...
                    logger.add_image('val/disp_coarse', disp0[:, :, None], global_step, dataformats='HWC')
            ...
            if i % args.i_video == 0:
                ...
                if args.use_viewdirs:
                    render_kwargs_test['c2w_static_cam'] = render_poses[0][:3, :4]
                    with torch.no_grad():
                        rgbs_still, _ = render_path(render_poses, hwf, K, args.chunk, render_kwargs_test,
                                                    render_factor=args.render_factor)
                    render_kwargs_test['c2w_static_cam'] = None
                    imageio.mimwrite(movie_base + 'rgb_still.mp4', to8b(rgbs_still), fps=30, quality=8)

            if i % args.i_testset == 0:
                testsavedir = os.path.join(basedir, expname, 'testset_{:06d}'.format(i))
                os.makedirs(testsavedir, exist_ok=True)
                os.makedirs(f'{testsavedir}_disp', exist_ok=True)
